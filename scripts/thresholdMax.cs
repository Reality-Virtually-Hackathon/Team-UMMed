...
public class thresholdMax : MonoBehaviour, IInputHandler, IInputClickHandler, IFocusable, IManipulationHandler
{
    ...
    public void OnInputClicked(InputEventData eventData)
    {
        ...
        {
            gameObject.transform.localPosition = new Vector3(gameObject.transform.localPosition.x, gameObject.transform.localPosition.y, -3.39f);
        } 


        // change threshold Max
        float dMax = dicom_cube.GetDMax();
        if (dMax >= 0.5f && dMax <= 0.9f)
        {
            dicom_cube.SetThresholdMax(dMax + 0.1f);
        }
        else
        {
            dicom_cube.SetThresholdMax(0.5f);
        }
    ...
    public void ChangeThreshold(Vector3 positon)
    {
        ...
        // set min and max 
        if (targetPosition.z >= -3.39f && targetPosition.z <= -2.76f)
        {
            ...
            dicom_cube.SetThresholdMin((transform.localPosition.x - (-3.39f)) / 0.315f);
        }
    }

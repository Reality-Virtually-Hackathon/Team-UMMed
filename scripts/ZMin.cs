...
public class ZMin : MonoBehaviour, IInputHandler, IInputClickHandler, IFocusable, IManipulationHandler, IHoldHandler
{
    ...
    public void OnInputClicked(InputEventData eventData)
    {
        ...
        {
            gameObject.transform.localPosition = new Vector3(gameObject.transform.localPosition.x, gameObject.transform.localPosition.y, 0.52f);
        }


        // change threshold Max
        float dMax = dicom_cube.GetYMin();
        if (dMax >= 0.0f && dMax <= 0.4f)
        {
            dicom_cube.SetYMin(dMax + 0.1f);
        }
        else
        {
            dicom_cube.SetYMin(0.0f);
        }
    ...
    public void ChangeThreshold(Vector3 positon)
    {
        ...
        // set min and max 
        if (targetPosition.z >= 0.52f && targetPosition.z <= 1.35f)
        {
            ...
            dicom_cube.SetThresholdMin((transform.localPosition.x - (0.52f)) / 0.415f);
        }
    }

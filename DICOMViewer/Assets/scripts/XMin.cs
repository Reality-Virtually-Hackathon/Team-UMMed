...
public class XMin : MonoBehaviour, IInputHandler, IInputClickHandler, IFocusable, IManipulationHandler
{
    ...
    public void OnInputClicked(InputEventData eventData)
    {
        ...
        {
            gameObject.transform.localPosition = new Vector3(gameObject.transform.localPosition.x, gameObject.transform.localPosition.y, -7.22f);
        }


        // change threshold Max
        float dMax = dicom_cube.GetXMin();
        if (dMax >= 0.0f && dMax <= 0.4f)
        {
            dicom_cube.SetXMin(dMax + 0.1f);
        }
        else
        {
            dicom_cube.SetXMin(0.0f);
        }
    ...
    public void ChangeThreshold(Vector3 positon)
    {
        ...
        // set min and max 
        if (targetPosition.z >= -7.22f && targetPosition.z <= -6.58f)
        {
            ...
            dicom_cube.SetThresholdMin((transform.localPosition.x - (-7.22f)) / 0.32f);
        }
    }

...
public class XMax : MonoBehaviour, IInputHandler, IInputClickHandler, IFocusable, IManipulationHandler
{
    ...
    public void OnInputClicked(InputEventData eventData)
    {
        ...
        {
            gameObject.transform.localPosition = new Vector3(gameObject.transform.localPosition.x, gameObject.transform.localPosition.y, -6.58f);
        }


        // change threshold Max
        float dMax = dicom_cube.GetXMax();
        if (dMax >= 0.5f && dMax <= 0.9f)
        {
            dicom_cube.SetXMax(dMax + 0.1f);
        }
        else
        {
            dicom_cube.SetXMax(0.5f);
        }
    ...
    public void ChangeThreshold(Vector3 positon)
    {
        ...
        // set min and max 
        if (targetPosition.z >= -6.58f && targetPosition.z <= -5.76f)
        {
            ...
            dicom_cube.SetThresholdMin((transform.localPosition.x - (-6.58f)) / 0.41f);
        }
    }

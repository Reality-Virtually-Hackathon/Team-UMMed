...
public class ZMax : MonoBehaviour, IInputHandler, IInputClickHandler, IFocusable, IManipulationHandler, IHoldHandler
{
    ...
    public void OnInputClicked(InputEventData eventData)
    {
        ...
        {
            gameObject.transform.localPosition = new Vector3(gameObject.transform.localPosition.x, gameObject.transform.localPosition.y, 1.35f);
        }


        // change threshold Max
        float dMax = dicom_cube.GetYMax();
        if (dMax >= 0.5f && dMax <= 0.9f)
        {
            dicom_cube.SetYMax(dMax + 0.1f);
        }
        else
        {
            dicom_cube.SetYMax(0.5f);
        }
    ...
    public void ChangeThreshold(Vector3 positon)
    {
        ...
        // set min and max 
        if (targetPosition.z >= 1.35f && targetPosition.z <= 1.94f)
        {
            ...
            dicom_cube.SetThresholdMin((transform.localPosition.x - (1.35f)) / 0.295f);
        }
    }

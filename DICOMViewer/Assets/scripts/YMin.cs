...
public class YMin : MonoBehaviour, IInputHandler, IInputClickHandler, IFocusable, IManipulationHandler
{
    ...
    public void OnInputClicked(InputEventData eventData)
    {
        ...
        {
            gameObject.transform.localPosition = new Vector3(gameObject.transform.localPosition.x, gameObject.transform.localPosition.y, -2.46f);
        }


        // change threshold Max
        float dMax = dicom_cube.GetZMin();
        if (dMax >= 0.0f && dMax <= 0.4f)
        {
            dicom_cube.SetZMin(dMax + 0.1f);
        }
        else
        {
            dicom_cube.SetZMin(0.0f);
        }
    ...
    public void ChangeThreshold(Vector3 positon)
    {
        ...
        // set min and max 
        if (targetPosition.z >= -2.46f && targetPosition.z <= -1.65f)
        {
            ...
            dicom_cube.SetThresholdMin((transform.localPosition.x - (-2.46f)) / 0.405f);
        }
    }

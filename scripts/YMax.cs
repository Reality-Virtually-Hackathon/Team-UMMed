...
public class YMax : MonoBehaviour, IInputHandler, IInputClickHandler, IFocusable, IManipulationHandler
{
    ...
    public void OnInputClicked(InputEventData eventData)
    {
        ...
        {
            gameObject.transform.localPosition = new Vector3(gameObject.transform.localPosition.x, gameObject.transform.localPosition.y, -1.61f);
        }


        // change threshold Max
        float dMax = dicom_cube.GetZMax();
        if (dMax >= 0.5f && dMax <= 0.9f)
        {
            dicom_cube.SetZMax(dMax + 0.1f);
        }
        else
        {
            dicom_cube.SetZMax(0.5f);
        }
    ...
    public void ChangeThreshold(Vector3 positon)
    {
        ...
        // set min and max 
        if (targetPosition.z >= -1.61f && targetPosition.z <= -1.03f)
        {
            ...
            dicom_cube.SetThresholdMin((transform.localPosition.x - (-1.61f)) / 0.29f);
        }
    }
